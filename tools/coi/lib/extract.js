...
// Kept in sync with netlify/functions/lib/extract.mjs (the production ESM
// port). Split into two concurrent calls (core policy data + checklist
// findings) instead of one combined call — a single call reading a real
// multi-page policy PDF and writing out both the structured data AND seven
// detailed checklist citations took 55-59 seconds, and Netlify's real
// synchronous function ceiling turned out to be 30 seconds (not the 60s
// originally assumed), so production now runs extraction as a Background
// Function regardless. The split still helps wall-clock time here too.
...
async function extractPolicyData(pdfBase64) {
